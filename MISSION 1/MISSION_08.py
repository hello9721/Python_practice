...
for i in m_num:                 # 1등부터 꼴등까지 순서대로 정렬된 m_num에서 하나씩 꺼내와서

    f.write("\n")               # 줄바꿈

    mean = i[0]                 # 평균은 i의 0번째
    n = m_num.index(i)          # 석차는 i의 인덱스

    for j in range(len(name)):  # name의 수만큼 반복을 하다가

        if i[1] == j:           # j가 i의 1번째와 같다면

            f.write(f"{name[j]},{k[j]},{m[j]},{e[j]},{c[j]},{total[j]},{mean},{n + 1}")
                                # 데이터 입력
f.close()                       # 전부 입력 후 파일 닫
